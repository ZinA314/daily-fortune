...
// 도 → "15°32′" 표기
export function degMin(x) {
  const dInt = Math.floor(x);
  const mInt = Math.floor((x - dInt) * 60);
  return `${dInt}°${String(mInt).padStart(2, '0')}′`;
}
...
export function calcNatalChart({ y, m, d, hour, minute, city, timeKnown }) {
  ...
  const positions = allLongitudes(dn);
  const nextPositions = allLongitudes(dn + 1); // 역행 판정용 (하루 뒤)

  const planets = PLANET_INFO.map((p) => {
    const lon = positions[p.key];
    const motion = rev(nextPositions[p.key] - lon);
    return {
      ...p,
      lon,
      retrograde: motion > 180, // 황경이 뒤로 가면 역행
      ...signOf(lon),
    };
  });

  // 평균 교점(노스 노드) — 항상 역행
  const nodeLon = rev(125.1228 - 0.0529538083 * dn);
  const node = {
    key: 'node', name: '노드', glyph: '☊', color: '#48484a',
    meaning: '카르마의 방향', desc: '이번 생에서 나아갈 성장의 방향',
    lon: nodeLon, retrograde: true, ...signOf(nodeLon),
  };
  ...
  // 하우스 배정 (등분 하우스 기준)
  const houseOf = (lon) => (asc == null ? null : Math.floor(rev(lon - asc) / 30) + 1);
  for (const p of planets) p.house = houseOf(p.lon);
  node.house = houseOf(node.lon);
  ...
  return { planets, node, asc, mc, houses, aspects, timeKnown };
}
